...
One way to plot conditional distributions for a discrete variable like this one might be to use a bar graph. Since there is a relatively large number of groups, it doesn't make sense to plot all the distributions on the same graph, so instead we can use a facet wrap:
...
However, we see that when there is a large number of groups, the individual graphs are quite small, making it difficult to interpret. We can improve the readability by using density functions instead of bar graphs. While this is technically incorrect because the variable at hand is not a continuous one, in practice it provides a solid, readable overview of the general shape of the distribution for each group:
